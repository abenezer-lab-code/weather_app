...
<button style={{backgroundColor:!isCelsius?"hsl(243,23%,24%)":"hsl(243, 27%, 20%)"}} className="block text-left mb-2 p-1 rounded" onClick={()=>{
  ...
  setisCelicius(false)
  }}>Fahrenheit<img hidden={isCelsius} src=".\public\assets\images\icon-checkmark.svg" aria-hidden className="inline ml-9"/></button>
</div>
...
setIsMph(false)
  }}>km/h <img hidden={ismph} src=".\public\assets\images\icon-checkmark.svg" aria-hidden className="inline ml-30"/></button>
  <button style={{backgroundColor:ismph?"hsl(243,23%,24%)":"hsl(243, 27%, 20%)"}}  className="block text-left mb-2 p-1 rounded"  onClick={()=>{
    setWindSpeedUnit("mph")
    setIsMph(true)
    }}>mph<img hidden={!ismph} src=".\public\assets\images\icon-checkmark.svg" aria-hidden className="inline ml-9" /></button>
</div>
...
<button style={{backgroundColor:!isInch?"hsl(243,23%,24%)":"hsl(243, 27%, 20%)"}}  aria-checked className="flex block text-left  p-1 rounded "  onClick={()=>{
  ...
  setIsInch(false)
  }}><span>Millimeter(mm)</span><img hidden={isInch} src=".\public\assets\images\icon-checkmark.svg" aria-hidden className="inline ml-9"/></button>
<button style={{backgroundColor:isInch?"hsl(243,23%,24%)":"hsl(243, 27%, 20%)"}}  className="block mb-2 text-left p-1 rounded" onClick={()=>{
  setPrecipitationUnit("inch")
  setIsInch(true)
  }}>inches<img hidden={!isInch} src=".\public\assets\images\icon-checkmark.svg" aria-hidden className="inline ml-9" /></button>
</div>
</div>
</div>
